...
class Order
{
    ...
    {
        $this->missingParams[$param->getCode()] = $param;
    ...
    public function status(){
        return $this->getActiveStatus()->getStatus();
    }
    ...
    public function deliveryFee(){
        return Globals::formaterPrix($this->getDeliveryFee());
    }

    public function getDeliveryFee(){
        if ($this->getType()->getId() == 2){
            foreach ($this->restaurant->getTownsDeliveredList() as $fee){
                if ($fee->getDeliveryTown() == $this->clientTown){
                    return $fee->getDeliveryFee();
                }
            }
        }
        return null;
